sap.ui.define([
	...
	var PageController = Controller.extend("com.sapJITMonitor.controller.GoalsBlock", {

		// Setup crumb info, the collection root
		// and an initial product selection / order state
		sCollection: "/ProductHierarchy",
		aCrumbs: ["ComponentGroups", "Materials"],
		mInitialOrderState: {
			products: {},
			count: 0,
			hasCounts: false
		...
		onInit: function (oEvent) {
			...
			var sPath = jQuery.sap.getModulePath("com.sapJITMonitor.model", "/productHierarchy.json");
			var oModel = new JSONModel(sPath);
			this.getView().setModel(oModel);
			this.getView().setModel(new JSONModel(this.mInitialOrderState), "Order");

			if (!this.oTemplate) {
				this.oTemplate = sap.ui.xmlfragment("com.sapJITMonitor.fragments.Row");
			}
